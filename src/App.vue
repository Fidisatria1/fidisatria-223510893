  ...
  <div class="todo-app">
    ...
    <ul>
      <li v-for="(todo, index) in filteredTodos" :key="index" :class="{ done: todo.done }">
        ...
        <input v-if="todo.editMode" type="text" v-model="todo.text" @blur="updateTodoText(index)" />
        <span v-else>{{ todo.text }}</span>
        <button @click="toggleEditMode(todo)">Update</button>
